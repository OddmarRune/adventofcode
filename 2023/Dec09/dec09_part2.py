...
def main():
    ...
    with open("input.txt", encoding="UTF-8") as file:
        ...
        for line in (tmp.strip() for tmp in file.readlines()):      # Added this [::-1]
            numbers = [[int(item) for item in line.split() if item.strip() != ''][::-1]]
        ...
        print(f" Part 2 : The sum of extrapolated values are {sum_of_next_numbers}")
